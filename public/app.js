...
const flags={india:"🇮🇳",australia:"🇦🇺",england:"🏴",pakistan:"🇵🇰",south:"🇿🇦",bangladesh:"🇧🇩",new:"🇳🇿",zealand:"🇳🇿",sri:"🇱🇰",lanka:"🇱🇰",afghanistan:"🇦🇫",ireland:"🇮🇪",scotland:"🏴",zimbabwe:"🇿🇼",nepal:"🇳🇵",netherlands:"🇳🇱"};
function teamFlag(t){const c=normalize(teamCountry(t)||teamName(t));const k=Object.keys(flags).find(x=>c.includes(x));return k?flags[k]:""}
function teamLogo(t){return typeof t==="object"?t?.logo_url||t?.logo||t?.image||"":""}
function scorePart(v){if(v==null||v==="")return"—";if(typeof v==="object")return v.runs!=null?`${v.runs}/${v.wickets??0}`:"—";return String(v)}
function scoreText(m,s){const score=m.score;if(!score||typeof score!=="object")return score==null?"—":String(score);return scorePart(score[s])}
function oversText(m,s){const source=m.overs||m.score?.overs||m.innings;if(typeof source==="object"&&source?.[s]!=null)return`${source[s]} Ov`;if(m[`${s}_overs`]!=null)return`${m[`${s}_overs`]} Ov`;return"—"}
function runRate(m,s){const source=m.run_rate||m.runRate||m.score?.run_rate;if(typeof source==="object"&&source?.[s]!=null)return Number(source[s]).toFixed(2);if(typeof source==="number")return source.toFixed(2);return""}
function formatDate(v){const d=new Date(v);return Number.isNaN(d.getTime())?String(v||""):d.toLocaleString([],{dateStyle:"medium",timeStyle:"short"})}
function escapeHtml(v){return String(v??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]))}
function venueText(m){const v=m.venue||m.stadium||m.ground||{};if(typeof v==="string")return[v,m.city||m.country].filter(Boolean);return[v.name||"Venue TBC",v.city||v.location||v.country||""].filter(Boolean)}
function situationText(m){return m.situation||m.match_situation||m.description||""}
